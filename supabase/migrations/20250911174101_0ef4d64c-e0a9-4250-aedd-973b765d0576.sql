...
ADD COLUMN IF NOT EXISTS ical_urls TEXT[] DEFAULT '{}',
ADD COLUMN IF NOT EXISTS ical_validation_status TEXT DEFAULT 'pending',
...
DROP TRIGGER IF EXISTS update_ical_events_updated_at ON public.ical_events;
CREATE TRIGGER update_ical_events_updated_at
BEFORE UPDATE ON public.ical_events
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();

-- Add foreign key reference to bookings (with IF NOT EXISTS check)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'ical_events_booking_id_fkey'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'ical_events_job_id_fkey'
  ) THEN
    ALTER TABLE public.ical_events
